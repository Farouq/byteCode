...
//		if(methodData.equals(methodsData1)) 		System.out.println("equal");
	...
	public static void loadDataIntoArraylists(Configuration config) throws Exception {
		...
		methodData.clear();
		calledMethods.clear();
		methodsDataForCalls.clear();
		methodSigniture.clear();
		methodsDataForSignature.clear();
	...
	public static int detectClones(Configuration config, String reportName)throws Exception	
	{ 
		loadDataIntoArraylists(config);

		String outputFileAddress2=config.reportAddress+ "\\"+reportName+".xml";
		...
		for(int v =0; v<byteCode.size()-1;v++){
			...
			for(int c =v+1; c<byteCode.size(); c++){ //cscode.size();c++){
				...
				if(byteCode.get(v).length()<2000 && byteCode.get(c).length()<2000)
				{
					...
					if (finalSimilarity>=config.threshold || dInstruction>=0.9 || dSigniture>=0.9 ||  dMethod>=0.9  /*|| (dSigniture>=0.8 && dInstruction>=0.8)*/) {
