...
public class DetectClones {
	...
	public static void detectsimilarities(Configuration config)throws Exception	
	{ 
		...
		for(int v =0; v<byteCode.size()-1;v++){
			...
			for(int c =v+1; c<byteCode.size(); c++){ //cscode.size();c++){
				...
				if(byteCode.get(v).length()<2000 && byteCode.get(c).length()<2000)
				{
					...
					System.out.println("<clone_fragment file=\""+methodData.get(v).get(0)+"\" startline=\""+ methodData.get(v).get(1) +"\" endline=\""+ methodData.get(v).get(2)+"\">");
				//	System.out.println(getsource( config, methodData.get(v).get(0), methodData.get(v).get(1)));
					
					System.out.println("<clone_fragment file=\""+methodData.get(c).get(0)+"\" startline=\""+ methodData.get(c).get(1) +"\" endline=\""+ methodData.get(c).get(2)+"\">");
				//	System.out.println(getsource( config, methodData.get(v).get(0), methodData.get(v).get(1)));
					System.out.println(methodSigniture.get(v)+"   %%%%   "+ methodSigniture.get(c));

					System.out.println("Instruction similarity is : " + dInstruction);
					System.out.println("Method Calls similarity is : " + dMethod);
					System.out.println("Signitaure similarity is : " + dSigniture);
				
					System.out.println("Final similarity is : " + finalSimilarity);
					System.out.println(" # of called methods"+calledMethods.get(v).size()+" "+ calledMethods.get(v)+"  %%%%%%%%%%"+  " # of called methods"+calledMethods.get(c).size()+" "+ calledMethods.get(c));
					}
